...
import { getFirestore, collection, getDocs, query, where, setDoc, doc, Timestamp,DocumentData } from 'firebase/firestore';
...
const Quiz = ({ route }: QuizProps) => {
  ...
  const saveAnalytics = async () => {
    try {
      ...
      const analyticsRef = doc(Firestore, 'userAnalytics', 'userId');
  ...
  const handleNextQuestion = () => {
    if (currentQuestion === quizData.length - 1) {
      saveAnalytics();
      setFeedback('Quiz completed! Check your analytics.');
    } else {
